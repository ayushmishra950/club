...
export const markAsReadByUser = async (req, res) => {
    try {
        const { id } = req.params;
        const suggestion = await Suggestion.findByIdAndUpdate(id, { $set: { userUnreadCount: 0 } }, { new: true }).populate("createdBy", "fullName email profileImage");
        if (!suggestion) {
            return res.status(404).json({ message: "Suggestion not found." });
        }
        const io = getIO();
        ...
        return res.status(200).json({
            message: "Suggestion marked as read by user",
            data: suggestion,
        });
    }
    catch (error) {
